...
from cli.lundump import Chunk, Instruction, Opcodes, readRKasK, whichRK


class _Scope:
  def __init__(self, startPC: int, endPC: int) -> None:
...
class _Line:
  def __init__(self, startPC: int, endPC: int, src: str, scope: int) -> None:
    self.startPC = startPC
    self.endPC = endPC
    self.src = src
    self.scope = scope
...
class LuaDecomp:
  def __init__(self, chunk: Chunk, headChunk: bool = True, scopeOffset: int = 0) -> None:
    self.chunk = chunk
    self.pc = 0
  ...
  def __makeTracIfNotExist(self) -> None:
    if self.pc not in self.traceback:
  ...
  def __condJmp(self, op: str, rkBC: bool = True) -> None:
    ...
    if instr.A > 0:
      self.__addExpr(f"{jmpType} not ")
    else:
      self.__addExpr(f"{jmpType} ")
    ...
    if scopeStart:
      self.__startScope(f"{scopeStart} ", self.pc - 1, jmp)
